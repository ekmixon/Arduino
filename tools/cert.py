...
def printData(data, showPub = True):
    try:
        ...
        try:
            ...
            if len(xcert) > 1:
                print(f'// Warning: TODO: pkcs7 has {len(xcert)} entries')
            xcert = xcert[0]
    ...
    if showPub:
        ...
        print(f'const char fingerprint_{name} [] PROGMEM = "{fingerprint}";')

        pem = xcert.public_key().public_bytes(Encoding.PEM, PublicFormat.SubjectPublicKeyInfo).decode('utf-8')
        print(f'const char pubkey_{name} [] PROGMEM = R"PUBKEY(')
        print(pem + ')PUBKEY";')

    else:

        cert = xcert.public_bytes(Encoding.PEM).decode('utf-8')
        print(f'const char cert_{name} [] PROGMEM = R"CERT(')
        print(cert + ')CERT";')
...
def get_certificate(hostname, port, name):
    ...
    with socket.create_connection((hostname, port)) as sock:
        with context.wrap_socket(sock, server_hostname=hostname) as ssock:
            ...
            print()
            if name:
                print(f'const char* {name}_host = "{hostname}";');
